...
9. Validate coverage, sizes, frames, durations, disposal, loops, safe-area containment, template-local alignment groups, rendered-ink collisions with both elements and fixed obstacles, unnecessary wrapping, and minimum readable font sizes. Generate a QA grid for every template so variants can be compared as a matrix rather than one file at a time.
